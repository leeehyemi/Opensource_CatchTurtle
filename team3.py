...
screen = t.Screen() # 화면 
score_board = t.Turtle() # 점수 표시

shape_list = ["turtle", "circle", "triangle", "square"] # 추가 : 캐릭터 모양 추가
player.shape(shape_list[0]) 
player.speed(0) # 캐릭터 이동 속도 
player.up() # 거북이의 펜을 든 상태에서 선을 그리지 않고 이동 
...
def show_message(up, down): # 메세지 출력
    player.goto(0,200)
    player.write(up, False, "center", ("Arial", 20, "bold")) #up 출력
    player.goto(0,-200)
    player.write(down, False, "center", ("Arial",15, "normal")) #down 출력
    player.goto(0,0)
    player.showturtle() # 거북이를 화면에 표시
...
def game_play(playing): # 게임 진행 함수
    ...
    maximum = time.time() + 30 # 30초 제한 

    while playing:
        global life # 생명 
        if time.time() > maximum or life <= 0:
            # 30초 초과하거나 생명이 0이하일 경우 종료
            playing = False
            return playing
        ...
        sleep_time = random.uniform(1, 2) # 1과 2사이의 무작위 소수점 숫자 생성, 할당
...
def turn_up(): # 위쪽 방향키 함수
    ...
    if player.position() == (0.00, 200.00):
        ...
        player.hideturtle() #거북이 위치는 그대로 두고 회면에 안보이게
        show_life(life)
...
photo = PhotoImage(file=r"C:/Users/annie/OneDrive/바탕 화면/전공, 교양/계절학기/오픈소스/tur_project_final/oss_project/images(1).png", master=root)



label1 = Label(root,width = 450,height = 450,relief = "solid",borderwidth = 10,padx = 5, pady = 10,image = photo)
label1.pack() # 현재 창에 배치, root 윈도우에 표시

def rule():
    root = Tk() #Tkinter 애플리케이션의 기본 윈도우를 생성 
